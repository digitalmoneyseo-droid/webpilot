...
export function WorkDetailPage({ locale, project, related }: { locale: Locale; project: ProjectEntry; related: ProjectEntry[] }) {
  ...
  const narratives = [
    [t(locale, "work.challenge"), p.challenge],
    [t(locale, "work.strategy"), p.strategy],
    [t(locale, "work.execution"), p.execution],
  ] as const;
  ...
  return (
    <main id="main-content">
      <article className="case-study">
        ...
        <dl className="case-summary reveal grid grid-cols-[1.5fr_1fr_1fr] gap-3 px-page pt-6 pb-section-compact max-[900px]:grid-cols-2 max-[600px]:grid-cols-1" data-reveal>
          <div className="flex min-h-[210px] flex-col rounded-card bg-surface p-6 shadow-surface max-[900px]:col-span-full">
            <dt className="text-caption text-[#888]">{t(locale, "work.result")}</dt>
          ...
          <div className="flex min-h-[210px] flex-col rounded-card bg-surface p-6 shadow-surface">
            <dt className="text-caption text-[#888]">{t(locale, "work.timeline")}</dt>
            <dd className="mt-auto text-heading-md">{p.timeline}</dd>
          </div>
          <div className="flex min-h-[210px] flex-col rounded-card bg-surface p-6 shadow-surface">
            <dt className="text-caption text-[#888]">{t(locale, "work.services")}</dt>
            <dd className="mt-auto text-body text-[#555]">{p.categories.join(", ")}</dd>
          </div>
        </dl>
        ...
        <section className="case-tools reveal mx-auto grid max-w-layout grid-cols-[200px_1fr] border-b border-line py-section-compact max-[600px]:grid-cols-1 max-[600px]:gap-6" data-reveal>
          <span className="font-mono text-label uppercase">{t(locale, "work.technology")}</span>
      ...
      <section className="related-work px-page py-section">
        <SectionHeading title={t(locale, "work.next")} align="center" />
