...
export function LocaleShell({ locale, pathname, children }: { locale: Locale; pathname: string; children: React.ReactNode }) {
  return (
    <>
      <ScrollProgress />
      <a className="fixed top-3 left-4 z-[1000] -translate-y-[160%] rounded-full bg-[#111] px-3.5 py-2.5 text-white focus:translate-y-0" href="#main-content">{t(locale, "a11y.skip")}</a>
      <ClientEffects />
