  ...
  return (
    <>
      <header className="fixed top-[22px] left-1/2 z-50 flex w-max -translate-x-1/2 items-center gap-2 max-[900px]:left-[15px] max-[900px]:[translate:none]">
        <Link href={localizePath("/", locale)} className="group/brand inline-flex h-11 items-center justify-center rounded-[12px] bg-white px-[17px] shadow-surface transition-transform duration-150 active:scale-[.96] motion-reduce:transition-none motion-reduce:active:scale-100" aria-label={t(locale, "Webpilot home")}><BrandMark /></Link>
        <nav className="relative inline-flex h-11 items-center justify-center gap-px rounded-[12px] bg-white p-1 shadow-surface max-[900px]:hidden" aria-label={t(locale, "Main navigation")}>
          {navItems.map((item) => <Link key={item.href} href={localizePath(item.href, locale)} className={`relative z-1 inline-flex h-9 items-center rounded-lg px-3.5 text-navigation transition-[color,background-color,scale] duration-150 active:scale-[.96] motion-reduce:transition-none motion-reduce:active:scale-100 ${isActive(item.href) ? "bg-[var(--ds-gray-alpha-100)] text-[var(--ds-gray-1000)]" : "text-[var(--ds-gray-900)] hover:bg-[var(--ds-gray-alpha-100)] hover:text-[var(--ds-gray-1000)]"}`} aria-current={isActive(item.href) ? "page" : undefined}>{item.label}</Link>)}
        </nav>
        <LanguageLink />
        <button ref={openRef} className="fixed top-[22px] right-[15px] inline-flex size-11 items-center justify-center rounded-[12px] bg-white shadow-surface transition-transform duration-150 active:scale-[.96] motion-reduce:transition-none motion-reduce:active:scale-100 min-[901px]:hidden" type="button" onClick={() => setOpen(true)} aria-label={t(locale, "Open navigation")} aria-expanded={open} aria-controls="site-menu"><Menu className="w-[17px]" strokeWidth={1.7} /></button>
      </header>
      <div id="site-menu" className={`fixed inset-0 z-100 flex flex-col overscroll-contain bg-[#101010] px-[clamp(24px,5vw,76px)] pt-6 pb-[34px] text-white transition-[opacity,transform,visibility] duration-300 ease-[var(--ease-out)] min-[901px]:hidden max-[900px]:overflow-y-auto max-[600px]:px-[18px] motion-reduce:transition-none ${open ? "visible translate-y-0 opacity-100" : "invisible -translate-y-3 opacity-0"}`} aria-hidden={!open} inert={!open} role="dialog" aria-modal="true" aria-label={t(locale, "Site navigation")}>
