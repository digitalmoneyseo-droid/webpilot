...
export function CtaButton({ href, light = false, className = "", children }: { href: string; light?: boolean; className?: string; children: React.ReactNode }) {
  return (
    <Link className={`pill-button inline-flex min-h-[52px] items-center gap-3.5 rounded-pill py-0 pr-2.5 pl-4 text-control shadow-dark-surface max-[600px]:justify-between ${light ? "pill-button--light bg-white text-ink" : "bg-dark text-white"} ${className}`} href={href}>
