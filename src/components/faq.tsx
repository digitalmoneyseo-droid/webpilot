      ...
      <div className="relative m-0 w-full">
        {items.map((item, index) => {
          ...
          return <div className={`relative before:pointer-events-none before:absolute before:inset-x-0 before:top-0 before:z-1 before:h-px before:bg-[var(--ds-gray-alpha-200)] before:content-[''] last:after:pointer-events-none last:after:absolute last:after:inset-x-0 last:after:bottom-0 last:after:z-1 last:after:h-px last:after:bg-[var(--ds-gray-alpha-200)] last:after:content-[''] ${expanded ? "[&_.faq-toggle_svg]:rotate-90" : ""}`} key={itemId}>
            <h3 className="m-0">
              <button className="grid min-h-21 w-full cursor-pointer grid-cols-[1fr_24px] items-center gap-4 rounded-md bg-transparent px-2 py-5 text-left transition-colors duration-150 max-[600px]:min-h-18" id={`${itemId}-button`} type="button" aria-expanded={expanded} aria-controls={`${itemId}-answer`} onClick={() => setOpen(expanded ? null : index)}>
                <span className="faq-question text-heading-sm">{item.data.question}</span>
                <span className="faq-toggle grid size-6 place-items-center justify-self-end" aria-hidden="true"><ChevronRight className="size-5 transition-transform duration-200 ease-[cubic-bezier(.4,0,.2,1)] motion-reduce:transition-none" strokeWidth={1.7} /></span>
              </button>
            </h3>
