    ...
    <footer className="mt-auto bg-black px-page pt-16 pb-8 text-inverse">
      <div className="footer-main flex min-h-40 justify-between gap-12 max-[600px]:block">
        ...
        <div className="footer-links flex gap-20 max-[600px]:mt-16 max-[600px]:grid max-[600px]:grid-cols-2 max-[600px]:gap-x-6">
          <div className="flex min-w-25 flex-col gap-2"><span className="mb-3 text-meta text-[#929292]">{t(locale, "footer.explore")}</span><Link className="text-meta hover:text-white" href={localizePath("/solutions", locale)}>{t(locale, "nav.solutions")}</Link><Link className="text-meta hover:text-white" href={localizePath("/portfolio", locale)}>{t(locale, "nav.portfolio")}</Link><Link className="text-meta hover:text-white" href={localizePath("/about", locale)}>{t(locale, "nav.about")}</Link></div>
