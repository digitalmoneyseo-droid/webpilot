...
export function OfferOverview({ locale }: { locale: Locale }) {
  const offers = locale === "de" ? [
    ...
    ["Integrated Partnership", "Foundation, Optimization, and Campaign are coordinated through one shared roadmap.", "Businesses that need several digital areas to be planned, delivered, and improved together.", "One coordinated working rhythm with clear priorities instead of disconnected initiatives.", "How the partnership works", "One shared roadmap connects strategy, delivery, testing, and ongoing prioritization across every relevant area.", ["Foundation", "Optimization", "Campaign"]],
  ] as const;
  ...
  return <div className="mx-auto max-w-[70rem]">{offers.map((offer, index) => {
    const [title, intro, bestFor, outcome, detailLabel, detail] = offer;
    return <Offer key={title} locale={locale} index={index} title={title} intro={intro} rows={[[locale === "de" ? "Für wen es ideal ist" : "Who it is best for", bestFor], [locale === "de" ? "Welches Ergebnis du erwarten kannst" : "The outcome you can expect", outcome], [detailLabel, detail]]} theme={themes[index]!} />;
  })}</div>;
}

function Offer({ locale, index, title, intro, rows, theme }: { locale: Locale; index: number; title: string; intro: string; rows: readonly (readonly [string, string])[]; theme: string }) {
...
function OptimizationSearchAnimation({ locale }: { locale: Locale }) {
  ...
  const query = locale === "de" ? "Beste Agentur für digitales Wachstum" : "Best digital growth agency";
  ...
  const resultDescriptions = locale === "de"
    ? ["Strategie und digitale Kampagnen", "Digitale Erlebnisse für wachsende Marken", "Performance Marketing und Optimierung"]
    : ["Strategy and digital campaigns", "Digital experiences for growing brands", "Performance marketing and optimization"];
  ...
  return <div ref={containerRef} className="grid h-full min-h-0 w-full grid-rows-[auto_1fr]">
    <div>
      <div className="flex min-h-11 items-center gap-3 rounded-full border border-[var(--ds-gray-alpha-200)] bg-white px-4 shadow-[0_2px_8px_rgb(0_0_0/.06)] max-[430px]:min-h-9 max-[430px]:gap-2 max-[430px]:px-3">
        ...
        <span className="min-w-0 truncate text-[clamp(.72rem,1.25vw,.875rem)] text-[var(--ds-gray-1000)]">
          ...
          {typedLength < query.length ? <motion.span className="ml-px inline-block h-[1em] w-px translate-y-[.12em] bg-[var(--ds-blue-700)]" animate={{ opacity: [1, 0, 1] }} transition={{ duration: 0.7, repeat: Infinity }} /> : null}
        </span>
      </div>
      <motion.div className="mt-3 flex items-center justify-between px-1 text-[clamp(.58rem,1vw,.7rem)] text-[var(--ds-gray-700)] max-[430px]:mt-2" initial={false} animate={{ opacity: resultsVisible ? 1 : 0 }} transition={{ duration: 0.24 }}>
        <span>{locale === "de" ? "Suchergebnisse" : "Search results"}</span>
        <span>{locale === "de" ? "Rang" : "Rank"}</span>
      </motion.div>
    </div>
    ...
    <motion.div ref={resultsViewportRef} className="relative mt-4 min-h-0 [container-type:size] max-[430px]:mt-2" initial={false} animate={{ opacity: resultsReady ? 1 : 0, y: resultsReady ? 0 : 6 }} transition={{ duration: 0.32, ease: [0.16, 1, 0.3, 1] }}>
      ...
      <motion.ol
        ...
        initial={false}
        animate={{ y: flightStarted ? 0 : -resultStackOffset }}
        transition={{ y: { duration: prefersReducedMotion || !flightStarted ? 0 : OPTIMIZATION_FLIGHT_DURATION, ease: [0.9, 0, 0.1, 1] } }}
      >
      ...
      <motion.div
        ...
        initial={false}
        animate={{ y: flightStarted ? 0 : winnerStartOffset }}
        style={{ height: baseResultHeight }}
        transition={{
          y: { duration: prefersReducedMotion || !flightStarted ? 0 : OPTIMIZATION_FLIGHT_DURATION, ease: [0.9, 0, 0.1, 1] },
        }}
      >
        ...
        <div className="absolute inset-0 z-30 grid grid-cols-[minmax(0,1fr)_2.25rem] items-center gap-3 px-3 py-2 transition-[padding] duration-500 ease-[cubic-bezier(.16,1,.3,1)] max-[430px]:py-1">
          <div className="min-w-0">
            ...
            <p className={`mt-1 mb-0 truncate leading-tight transition-[color,font-size] duration-300 max-[430px]:hidden ${winnerLanded ? "text-[clamp(.64rem,1vw,.74rem)] text-[#796b58]" : "text-[clamp(.62rem,1vw,.72rem)] text-[var(--ds-gray-800)]"}`}>{locale === "de" ? "Digitale Systeme für profitables Wachstum" : "Digital systems for profitable growth"}</p>
          </div>
