...
export function ProjectCard({ project, featured = false, decorative = false }: { project: Project; featured?: boolean; decorative?: boolean }) {
  return (
    <Link href={`/work/${project.slug}`} className={`project-card${featured ? " project-card--featured" : ""}`} aria-hidden={decorative || undefined} tabIndex={decorative ? -1 : undefined}>
